...
# --- 設定 CSV 檔案路徑 ---
csv_file_path = "level3_model_inference_results.csv" # 替換成您的 CSV 檔案路徑
...
try:
    ...
    if len(true_labels) > 0 and len(predicted_labels) > 0:
        ...
        sns.heatmap(cm_df, annot=True, fmt='d',
                    ...
                    linewidths=0.8, 
                    linecolor='black', 
                    annot_kws={"size": 18, "color": "brown"}) 

        plt.title('混淆矩陣', fontsize=20) 
        plt.xlabel('預測標籤', fontsize=16) 
        plt.ylabel('實際標籤', fontsize=16)
...
# 讀取CSV檔案
#df = pd.read_csv("level3_model_inference_results.csv") # 請確認這是你實際使用的CSV檔名

# 數據清洗：移除頭尾空格，將 'nan' 字串替換為真正的 NaN
#df["Model_Predicted_Label"] = df["Model_Predicted_Label"].astype(str).str.strip().replace('nan', np.nan)
#df["Actual_Completion"] = df["Actual_Completion"].astype(str).str.strip().replace('nan', np.nan)

# 定義目標類別
target_classes = ["Low Risk", "High Risk"]

# 篩選出 Model_Predicted_Label 和 Actual_Completion 都在目標類別中，且兩者皆非空的數據
# 這是為了確保與混淆矩陣計算使用相同的數據子集

# 在這個已處理的資料集上計算不一致的樣本
mismatch_df = df_results[
    df_results["Model_Predicted_Label"] != df_results["Actual_Completion"]
]

# 定義每個level的關鍵詞對應 (保持不變)
...
mismatch_df["Prompt_Level"] = mismatch_df["Prompt"].apply(detect_level)

# 統計各 Level 的不一致數量
mismatch_counts = mismatch_df["Prompt_Level"].value_counts()

# 顯示結果
print("--- 不一致樣本計算結果 ---")
print(f"共讀取原始 {len(df_results)} 條記錄。")
print(f"其中 {len(df_results)} 條記錄被納入不一致樣本計算 (排除非預期標籤和空值)。")
print("\nModel_Predicted_Label 和 Actual_Completion 不一致的樣本數量（依不同提問級別，經統一篩選）:")
print(mismatch_counts)
print("Model_Predicted_Label 和 Actual_Completion 不一致的樣本數量（總數，經統一篩選）:")
print(mismatch_df.shape[0])

#%%
import pandas as pd

# 讀取CSV檔案
df = pd.read_csv("level3_model_inference_results.csv")
#print(df["Model_Predicted_Label"])
# 篩選出無法回答的樣本
na_df = df[df["Model_Predicted_Label"]=="Low Risk"]
